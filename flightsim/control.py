...
class SE3Control(object):
    def __init__(self, quad_params):
        ...
        # # _______Gains for Projects 1/2/3_________________
        # self.Kp = np.diag(np.array([15, 15, 15]))
        # self.Kd = 2*np.sqrt(self.Kp)
        # self.Kr = np.diag(np.array([550, 550, 550]))
        # self.Kw = np.diag(np.array([75, 75, 75]))

        # _______Gains for Lab 1/2/3_________________
        self.Kp = np.diag(np.array([5, 5, 14]))
        self.Kd = 2 * np.sqrt(self.Kp)
        self.Kr = np.diag(np.array([500, 500, 500]))
        self.Kw = np.diag(np.array([70, 70, 70]))


    def update(self, state, flat_output):
        """
        This function receives the current time, true state, and desired flat
        outputs. It returns the command inputs.

        Inputs:
            (deleted) t, present time in seconds
